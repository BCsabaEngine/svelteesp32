...
- **v3.3.0** — Internal optimization release; **generated headers are byte-identical to v3.2.4**, so there is nothing to migrate. Fixes an invalid `engine` in an RC file terminating the **Vite build process** instead of reporting a plugin error. Header generation no longer builds the data arrays it discards (peak memory drops by ~4× the payload size), the source-directory scan is no longer quadratic when a compression plugin emits a `.gz` beside every asset, and all four engines now share one set of C++ generation helpers instead of ESP-IDF keeping inline copies
