...
TypeScript CLI tool and Vite plugin converting frontend apps (Svelte, React, Angular, Vue) into C++ header files for ESP32/ESP8266 web servers. Gzip compression, ETag support, 4 engines. Node.js >= 22, npm >= 10. MIT licensed (`LICENSE` at repo root, mirrored in `package.json`'s `license` field).
...
- `src/doctor.ts` — `svelteesp32 doctor`/`check` (dispatched in `bin/index.js`, which strips the subcommand from `argv`). `runChecks()` is pure over the `getFiles()` map and returns `Finding[]`; `main()` parses with the normal `parseArguments()`, calls `getFiles` with `noIndexCheck: true` (a missing index is a finding, not an exception) and exits 1 on errors (or any finding with `--strict`, stripped from `argv` before parsing). Reuses `toDataName`/`findIdentifierCollisions` from `pipeline.ts` — never a second copy. The 512-char URI limit is esp_http_server's Kconfig default and applies to psychic/espidf only.
