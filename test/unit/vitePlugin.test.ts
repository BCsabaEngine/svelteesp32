...
describe('vitePlugin', () => {
  ...
  describe('plugin options mode', () => {
    ...
    it('should set configSource to vite', async () => {
      const { runPipeline } = await import('../../src/pipeline');
      ...
      const options = vi.mocked(runPipeline).mock.calls[0]?.[0];
      expect(options?.configSource).toBe('vite');
    });
    ...
    it('should throw when output is not provided in plugin options mode', () => {
      const plugin = svelteESP32({});
      plugin.configResolved(mockConfig);
      expect(() => plugin.closeBundle()).toThrow('output is required');
    });

    it('should not call loadRcFileConfig in plugin options mode', () => {
      const plugin = svelteESP32({ output: './output.h' });
      plugin.configResolved(mockConfig);
      plugin.closeBundle();

      expect(loadRcFileConfig).not.toHaveBeenCalled();
    });
  });

  describe('RC file mode', () => {
    it('should set configSource to rcfile', async () => {
      const { runPipeline } = await import('../../src/pipeline');
      vi.mocked(loadRcFileConfig).mockReturnValue({ outputfile: './rc-output.h' });
      ...
      const options = vi.mocked(runPipeline).mock.calls[0]?.[0];
      expect(options?.configSource).toBe('rcfile');
    });

    it('should use RC file values when called with no argument', async () => {
      const { runPipeline } = await import('../../src/pipeline');
      vi.mocked(loadRcFileConfig).mockReturnValue({
    ...
    it('should throw when RC file does not provide outputfile', () => {
      vi.mocked(loadRcFileConfig).mockReturnValue({});
    ...
    it('should use build.outDir as sourcepath fallback in RC file mode', async () => {
      const { runPipeline } = await import('../../src/pipeline');
      ...
      const options = vi.mocked(runPipeline).mock.calls[0]?.[0];
      expect(options?.sourcepath).toBe('vite-dist');
    });

    it('should accept a string path for a custom RC file', () => {
      vi.mocked(loadRcFileConfig).mockReturnValue({ outputfile: './output.h' });

      const plugin = svelteESP32('./custom.rc.json');
      plugin.configResolved(mockConfig);
      plugin.closeBundle();
    ...
    it('should use RC cachetimehtml and cachetimeassets', async () => {
      const { runPipeline } = await import('../../src/pipeline');
      vi.mocked(loadRcFileConfig).mockReturnValue({
        outputfile: './output.h',
        cachetimehtml: 300,
